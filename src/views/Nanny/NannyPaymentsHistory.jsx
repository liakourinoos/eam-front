...
import { fetchCompletedNannyPayments } from '../../FetchFunctions';
import Payment from './Payment';

export default function NannyPaymentsHistory() { // Corrected function declaration

    const { userData } = useAuth();
    const { data: payments, isLoading } = useQuery({
        queryFn: () => fetchCompletedNannyPayments(userData?.id),
        queryKey: ['paymentHistoryNanny', userData?.id],
        enabled: !!userData
    });
    ...
    return (
        <div className="w-auto flex-grow py-2 flex flex-col h-full">
            ...
            <div className="w-11/12 mt-5 mx-auto h-16 text-center rounded-md bg-gray-300 border-2 border-gray-500 flex items-center font-medium text-xl">
                <p className="w-1/3 text-center">Κωδικός</p>

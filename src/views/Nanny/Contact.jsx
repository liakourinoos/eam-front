...
export default function Contact({parentName,parentSurname,id,contactType,contactInfo,date}) {
    ...
    return(
        <div className='w-full mx-auto h-20 text-2xl rounded-md border-2 text-center border-gray-300 bg-white flex items-center font-medium  py-2'>
            {/* name */}
            <Link to={`/parentprofile/${id}`} className='w-1/3 h-full flex justify-center items-center hover:text-pallete-600'>{parentName} {parentSurname}</Link>
            <p className='w-1/3 h-full flex justify-center items-center '>{date}</p>
            <div className='w-1/3  h-full  flex items-center justify-center gap-3'>

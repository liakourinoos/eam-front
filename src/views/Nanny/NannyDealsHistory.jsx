...
import { fetchNannyDeals } from '../../FetchFunctions';
import ApplicationPreview from './ApplicationPreview';

export default function NannyDealsHistory() { // Corrected function declaration

    const { userData } = useAuth();
    const { data: deals, isLoading } = useQuery({
        queryFn: () => fetchNannyDeals(userData?.id),
        queryKey: ['nannyDeals', userData?.id],
        enabled: !!userData
    });
    ...
    return (
        <div className="w-auto flex-grow py-2 flex flex-col h-full">
            ...
            <div className="w-11/12 mt-5 mx-auto h-16 text-center rounded-md bg-gray-300 border-2 border-gray-500 flex items-center font-medium text-xl">
                <p className="w-1/5 text-center">Κωδικός</p>
                <p className='w-1/5 text-center'>Ονοματεπώνυμο Γονέα</p>
                <div className="w-1/5 flex items-center justify-center gap-1"
                    title="Οι ημερομηνίες εμφανίζονται σε μορφή ΗΗ/ΜΜ/ΕΕΕΕ"
                >
                    <p>Ημερομηνία Λήψης Συμφωνητικού</p>
                    <FaRegQuestionCircle className="text-xl" />
                </div>
                <p className="w-1/5 text-center">Κατάσταση</p>
                <p className="w-1/5 text-center">Ενέργεια</p>
            </div>
            ...
            <div className='w-11/12 mx-auto flex flex-col mb-5 gap-2 items-center justify-start overflow-y-auto mt-2 flex-grow'>
                ...
                {!isLoading && deals?.length > 0 && Array.isArray(deals) &&
                    deals.map((deal, idx) => (
                        <ApplicationPreview key={idx} 
                            id={deal?.id} 
                            parentId={deal?.userId}
                            parentName={deal?.parentData.name}
                            parentSurname={deal?.parentData.surname}
                            date={deal?.finalizedAt}
